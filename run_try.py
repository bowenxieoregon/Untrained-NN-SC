...
from utils import matrix_normalize, outer, get_tensor, cost_func, nnls_my,NAE,get_tensor_tl,SRE
import random
...
in_size = (51,51)
net = get_net(input_depth, NET_TYPE, pad, upsample_mode, n_channels=1, act_fun='LeakyReLU', skip_n33d=256, skip_n33u=256, skip_n11=5, num_scales=6, downsample_mode='stride').to(device)
...
X_true = torch.from_numpy(T_true).to(device)

W = torch.from_numpy(Om).type(torch.float32)
X = torch.from_numpy(T_true_noise).type(torch.float32).to(device)
...
for i in range(inner):
    ...
    with torch.no_grad():
        ...
        var = es.emv
            ...
            # img = Image.open(buf)
        coss = cost_func(torch.tensor(T_true_noise).to(device),X_from_slf_temp, Wxxx).item()
        wandb.log({"loss": coss, "SRE": sre, "var": var, "mse":mse})
        # plt.close()
